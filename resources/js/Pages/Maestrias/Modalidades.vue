<template>
  <Head title="Modalidades de Maestría" />
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-2xl font-bold text-foreground">Modalidades de Maestría</h1>
        <p class="text-muted-foreground">
          Administra las modalidades disponibles para las maestrías.
        </p>
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Maestrías asociadas</TableHead>
          ...
          <TableBody>
            <TableRow v-for="modalidad in modalidades.data" :key="modalidad.id">
              ...
              <TableCell>{{ modalidad.maestrias_count ?? 0 }}</TableCell>
  ...
  <Dialog v-model:open="showFormDialog">
    <DialogContent class="sm:max-w-[420px]">
      <DialogHeader>
        ...
        <DialogDescription>
          {{ editingModalidad ? 'Modifique los datos de la modalidad seleccionada.' : 'Registre una nueva modalidad para las maestrías.' }}
        </DialogDescription>
      </DialogHeader>
...
import type { ModalidadMaestria, PaginatedResponse } from '@/types/titulos/maestria'
import { toast } from 'vue-sonner'

defineOptions({
  layout: (h: any, page: any) => h(AppLayout, {
    title: 'Modalidades Maestría',
    pageTitle: 'Modalidades Maestría',
    navTabs,
...
const props = defineProps<{
  modalidades: PaginatedResponse<ModalidadMaestria>
}>()

const page = usePage()
const showFormDialog = ref(false)
const showDeleteDialog = ref(false)
const editingModalidad = ref<ModalidadMaestria | null>(null)
const deletingModalidad = ref<ModalidadMaestria | null>(null)
...
const openEditDialog = (modalidad: ModalidadMaestria) => {
...
const submitForm = () => {
  if (editingModalidad.value) {
    form.put(route('maestrias.modalidades.update', editingModalidad.value.id), {
      onSuccess: () => {
    ...
    form.post(route('maestrias.modalidades.store'), {
      onSuccess: () => {
...
const openDeleteDialog = (modalidad: ModalidadMaestria) => {
...
const deleteModalidad = () => {
  ...
  router.delete(route('maestrias.modalidades.destroy', deletingModalidad.value.id), {
    onSuccess: () => {
