<template>
  <Head title="Menciones de Maestría" />
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-2xl font-bold text-foreground">Menciones de Maestría</h1>
        <p class="text-muted-foreground">
          Administra las menciones disponibles para las maestrías.
        </p>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            <TableRow v-for="mencion in menciones.data" :key="mencion.id">
              ...
              <TableCell>{{ mencion.maestrias_count ?? 0 }}</TableCell>
  ...
  <Dialog v-model:open="showFormDialog">
    <DialogContent class="sm:max-w-[420px]">
      <DialogHeader>
        ...
        <DialogDescription>
          {{ editingMencion ? 'Modifique los datos de la mención seleccionada.' : 'Registre una nueva mención para las maestrías.' }}
        </DialogDescription>
      </DialogHeader>
...
import type { MencionMaestria, PaginatedResponse } from '@/types/titulos/maestria'
import { toast } from 'vue-sonner'

defineOptions({
  layout: (h: any, page: any) => h(AppLayout, {
    title: 'Menciones Maestría',
    pageTitle: 'Menciones Maestría',
    navTabs,
    activeTab: 'menciones'
  }, () => page)
})
...
const props = defineProps<{
  menciones: PaginatedResponse<MencionMaestria>
}>()

const page = usePage()
const showFormDialog = ref(false)
const showDeleteDialog = ref(false)
const editingMencion = ref<MencionMaestria | null>(null)
const deletingMencion = ref<MencionMaestria | null>(null)
...
const openEditDialog = (mencion: MencionMaestria) => {
...
const submitForm = () => {
  if (editingMencion.value) {
    form.put(route('maestrias.menciones.update', editingMencion.value.id), {
      onSuccess: () => {
    ...
    form.post(route('maestrias.menciones.store'), {
      onSuccess: () => {
...
const openDeleteDialog = (mencion: MencionMaestria) => {
...
const deleteMencion = () => {
  ...
  router.delete(route('maestrias.menciones.destroy', deletingMencion.value.id), {
    onSuccess: () => {
