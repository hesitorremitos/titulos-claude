<template>
  <Head title="Menciones de Doctorado" />
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-2xl font-bold text-foreground">Menciones de Doctorado</h1>
        <p class="text-muted-foreground">
          Administra las menciones disponibles para los doctorados.
        </p>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            <TableRow v-for="mencion in menciones.data" :key="mencion.id">
              ...
              <TableCell>{{ mencion.doctorados_count ?? 0 }}</TableCell>
  ...
  <Dialog v-model:open="showFormDialog">
    <DialogContent class="sm:max-w-[420px]">
      <DialogHeader>
        ...
        <DialogDescription>
          {{ editingMencion ? 'Modifique los datos de la mención seleccionada.' : 'Registre una nueva mención para los doctorados.' }}
        </DialogDescription>
      </DialogHeader>
...
import type { MencionDoctorado, PaginatedResponse } from '@/types/titulos/doctorado'
import { toast } from 'vue-sonner'

defineOptions({
  layout: (h: any, page: any) => h(AppLayout, {
    title: 'Menciones Doctorado',
    pageTitle: 'Menciones Doctorado',
    navTabs,
    activeTab: 'menciones'
  }, () => page)
})
...
const editingMencion = ref<MencionDoctorado | null>(null)
const deletingMencion = ref<MencionDoctorado | null>(null)
...
const openEditDialog = (mencion: MencionDoctorado) => {
...
const submitForm = () => {
  if (editingMencion.value) {
    form.put(route('doctorados.menciones.update', editingMencion.value.id), {
      onSuccess: () => {
    ...
    form.post(route('doctorados.menciones.store'), {
      onSuccess: () => {
...
const openDeleteDialog = (mencion: MencionDoctorado) => {
...
const deleteMencion = () => {
  ...
  router.delete(route('doctorados.menciones.destroy', deletingMencion.value.id), {
    onSuccess: () => {
