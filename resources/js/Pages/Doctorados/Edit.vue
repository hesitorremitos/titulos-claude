<template>
  <Head :title="`Editar Doctorado - ${doctorado.persona?.nombres ?? ''} ${doctorado.persona?.paterno ?? ''}`" />
  <div class="space-y-6">
    <div class="border-b pb-4">
      <h1 class="text-2xl font-semibold text-foreground">
        Editar: {{ doctorado.persona?.nombres }} {{ doctorado.persona?.paterno }}
      </h1>
      ...
      <div class="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Modifique los datos específicos del doctorado.</CardDescription>
          </CardHeader>
          <CardContent class="space-y-4">
            <div class="grid grid-cols-2 gap-4">
              <div>
            ...
            <div>
              ...
              <Select v-model="form.mencion_doctorado_id">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select v-model="form.modalidad_doctorado_id">
          ...
          <CardContent>
            ...
            <div v-else class="text-center py-16">
              ...
              <p class="text-muted-foreground text-sm">
                Este doctorado no tiene un archivo PDF. Puede subir uno usando el formulario.
              </p>
            </div>
          </CardContent>
        </Card>
    ...
    <div class="flex justify-between pt-6 border-t">
      <Button variant="outline" as="a" :href="route('doctorados.show', doctorado.id)">
...
const form = useForm({
  ci: props.doctorado.ci ?? '',
  nombres: props.doctorado.persona?.nombres ?? '',
  paterno: props.doctorado.persona?.paterno ?? '',
  materno: props.doctorado.persona?.materno ?? '',
  nro_tpn: props.doctorado.nro_tpn ?? '',
  mencion_tpn_id: props.doctorado.mencion_tpn_id ? String(props.doctorado.mencion_tpn_id) : '',
  nro_documento: props.doctorado.nro_documento ?? '',
  fojas: props.doctorado.fojas ?? '',
  libro: props.doctorado.libro ?? '',
  fecha_emision: props.doctorado.fecha_emision ?? '',
  mencion_doctorado_id: props.doctorado.mencion_doctorado_id ? String(props.doctorado.mencion_doctorado_id) : '',
  gestion_inicial: props.doctorado.gestion_inicial ?? '',
...
const updateDoctorado = () => {
  form
    .transform((data) => normalizePayload(data))
    .post(route('doctorados.update', props.doctorado.id), {
      forceFormData: true,
      onSuccess: () => {
        toast.success('Doctorado actualizado exitosamente.')
      },
      onError: (errors) => {
        console.log('Validation errors:', errors)
        toast.error('Error al actualizar el doctorado. Revise los campos marcados.')
      }
    })
}
