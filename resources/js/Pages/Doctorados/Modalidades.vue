<template>
  <Head title="Modalidades de Doctorado" />
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-2xl font-bold text-foreground">Modalidades de Doctorado</h1>
        <p class="text-muted-foreground">
          Administra las modalidades disponibles para los doctorados.
        </p>
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Doctorados asociados</TableHead>
          ...
          <TableBody>
            <TableRow v-for="modalidad in modalidades.data" :key="modalidad.id">
              ...
              <TableCell>{{ modalidad.doctorados_count ?? 0 }}</TableCell>
  ...
  <Dialog v-model:open="showFormDialog">
    <DialogContent class="sm:max-w-[420px]">
      <DialogHeader>
        ...
        <DialogDescription>
          {{ editingModalidad ? 'Modifique los datos de la modalidad seleccionada.' : 'Registre una nueva modalidad para los doctorados.' }}
        </DialogDescription>
      </DialogHeader>
...
import type { ModalidadDoctorado, PaginatedResponse } from '@/types/titulos/doctorado'
import { toast } from 'vue-sonner'

defineOptions({
  layout: (h: any, page: any) => h(AppLayout, {
    title: 'Modalidades Doctorado',
    pageTitle: 'Modalidades Doctorado',
    navTabs,
...
const props = defineProps<{
  modalidades: PaginatedResponse<ModalidadDoctorado>
}>()

const page = usePage()
const showFormDialog = ref(false)
const showDeleteDialog = ref(false)
const editingModalidad = ref<ModalidadDoctorado | null>(null)
const deletingModalidad = ref<ModalidadDoctorado | null>(null)
...
const openEditDialog = (modalidad: ModalidadDoctorado) => {
...
const submitForm = () => {
  if (editingModalidad.value) {
    form.put(route('doctorados.modalidades.update', editingModalidad.value.id), {
      onSuccess: () => {
    ...
    form.post(route('doctorados.modalidades.store'), {
      onSuccess: () => {
...
const openDeleteDialog = (modalidad: ModalidadDoctorado) => {
...
const deleteModalidad = () => {
  ...
  router.delete(route('doctorados.modalidades.destroy', deletingModalidad.value.id), {
    onSuccess: () => {
