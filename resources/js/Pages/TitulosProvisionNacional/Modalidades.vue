      ...
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-muted-foreground">
            Gestione las modalidades de graduación disponibles para los títulos provisionales nacionales
          </p>
        </div>
    ...
    <Dialog v-model:open="showFormDialog">
      <DialogContent class="sm:max-w-[400px]">
        ...
        <div class="space-y-4 py-4">
          <div>
            <Label for="nombre">Modalidad de Graduación</Label>
            <Input
              id="nombre"
              v-model="form.nombre"
...
const submitForm = () => {
  if (editingModalidad.value) {
    // Update
    form.put(route('titulos-provision-nacional.modalidades.update', editingModalidad.value.id), {
      onSuccess: () => {
        closeFormDialog()
    ...
    // Create
    form.post(route('titulos-provision-nacional.modalidades.store'), {
      onSuccess: () => {
        closeFormDialog()
        toast.success((page.props.flash as any).success)
      },
...
const deleteModalidad = () => {
  if (deletingModalidad.value) {
    router.delete(route('titulos-provision-nacional.modalidades.destroy', deletingModalidad.value.id), {
      onSuccess: () => {
