...
import type { MencionTpn, PaginatedResponse } from '@/types/titulos/titulo-provision-nacional'
import type { Carrera } from '@/types/models'
...
const editingMencion = ref<MencionTpn | null>(null)
const deletingMencion = ref<MencionTpn | null>(null)
...
const openEditDialog = (mencion: MencionTpn) => {
...
const openDeleteDialog = (mencion: MencionTpn) => {
...
const submitForm = () => {
  if (editingMencion.value) {
    // Update
    form.put(route('titulos-provision-nacional.menciones.update', editingMencion.value.id), {
      onSuccess: () => {
        closeFormDialog()
    ...
    // Create
    form.post(route('titulos-provision-nacional.menciones.store'), {
      onSuccess: () => {
        closeFormDialog()
        toast.success((page.props.flash as any).success)
      },
...
const deleteMencion = () => {
  if (deletingMencion.value) {
    router.delete(route('titulos-provision-nacional.menciones.destroy', deletingMencion.value.id), {
      onSuccess: () => {
