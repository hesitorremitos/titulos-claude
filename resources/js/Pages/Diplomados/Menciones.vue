<template>
  <Head title="Menciones de Diplomado" />
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-2xl font-bold text-foreground">Menciones de Diplomado</h1>
        <p class="text-muted-foreground">
          Administra las menciones disponibles para los diplomados.
        </p>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            <TableRow v-for="mencion in menciones.data" :key="mencion.id">
              ...
              <TableCell>{{ mencion.diplomados_count ?? 0 }}</TableCell>
  ...
  <Dialog v-model:open="showFormDialog">
    <DialogContent class="sm:max-w-[420px]">
      <DialogHeader>
        ...
        <DialogDescription>
          {{ editingMencion ? 'Modifique los datos de la mención seleccionada.' : 'Registre una nueva mención para los diplomados.' }}
        </DialogDescription>
      </DialogHeader>
...
import type { MencionDiplomado, PaginatedResponse } from '@/types/titulos/diplomado'
import { toast } from 'vue-sonner'

defineOptions({
  layout: (h: any, page: any) => h(AppLayout, {
    title: 'Menciones Diplomado',
    pageTitle: 'Menciones Diplomado',
    navTabs,
    activeTab: 'menciones'
  }, () => page)
})
...
defineProps<{
  menciones: PaginatedResponse<MencionDiplomado>
}>()

const page = usePage()
const showFormDialog = ref(false)
const showDeleteDialog = ref(false)
const editingMencion = ref<MencionDiplomado | null>(null)
const deletingMencion = ref<MencionDiplomado | null>(null)
...
const openEditDialog = (mencion: MencionDiplomado) => {
...
const submitForm = () => {
  if (editingMencion.value) {
    form.put(route('diplomados.menciones.update', editingMencion.value.id), {
      onSuccess: () => {
    ...
    form.post(route('diplomados.menciones.store'), {
      onSuccess: () => {
...
const openDeleteDialog = (mencion: MencionDiplomado) => {
...
const deleteMencion = () => {
  ...
  router.delete(route('diplomados.menciones.destroy', deletingMencion.value.id), {
    onSuccess: () => {
