<template>
  <Head title="Modalidades de Especialidad" />
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-2xl font-bold text-foreground">Modalidades de Especialidad</h1>
        <p class="text-muted-foreground">
          Administra las modalidades disponibles para las especialidades.
        </p>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            <TableRow v-for="modalidad in modalidades.data" :key="modalidad.id">
              ...
              <TableCell>{{ modalidad.especialidades_count ?? 0 }}</TableCell>
  ...
  <Dialog v-model:open="showFormDialog">
    <DialogContent class="sm:max-w-[420px]">
      <DialogHeader>
        ...
        <DialogDescription>
          {{ editingModalidad ? 'Modifique los datos de la modalidad seleccionada.' : 'Registre una nueva modalidad para las especialidades.' }}
        </DialogDescription>
      </DialogHeader>
...
import type { ModalidadEspecialidad, PaginatedResponse } from '@/types/titulos/especialidad'
import { toast } from 'vue-sonner'

defineOptions({
  layout: (h: any, page: any) => h(AppLayout, {
    title: 'Modalidades Especialidad',
    pageTitle: 'Modalidades Especialidad',
    navTabs,
...
const props = defineProps<{
  modalidades: PaginatedResponse<ModalidadEspecialidad>
}>()

const page = usePage()
const showFormDialog = ref(false)
const showDeleteDialog = ref(false)
const editingModalidad = ref<ModalidadEspecialidad | null>(null)
const deletingModalidad = ref<ModalidadEspecialidad | null>(null)
...
const openEditDialog = (modalidad: ModalidadEspecialidad) => {
...
const submitForm = () => {
  if (editingModalidad.value) {
    form.put(route('especialidades.modalidades.update', editingModalidad.value.id), {
      onSuccess: () => {
    ...
    form.post(route('especialidades.modalidades.store'), {
      onSuccess: () => {
...
const openDeleteDialog = (modalidad: ModalidadEspecialidad) => {
...
const deleteModalidad = () => {
  ...
  router.delete(route('especialidades.modalidades.destroy', deletingModalidad.value.id), {
    onSuccess: () => {
